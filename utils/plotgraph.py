    ...
    for line in lines:
        ...
        if "#SH-period:" in line:
            ...
            try:
                ...
                icg_idx = next(
                    (i for i, p in enumerate(parts) if p == "ICG-period:"), -1
                )
                if icg_idx > 0 and icg_idx + 1 < len(parts):
                    metadata["icg_period"] = parts[icg_idx + 1]
                int_time_idx = next(
                    (i for i, p in enumerate(parts) if p == "time:"), -1
                )
        ...
        if "#Calibration-coefficients:" in line:
            ...
            try:
                ...
                if coeff_idx < len(parts):
                    ...
                    metadata["calibration_coeffs"] = [
                        float(c) for c in coeff_str.split(",")
                    ]
            except:
                pass
        ...
        if "#Regression-enabled:" in line:
            ...
            try:
                ...
                if enabled_idx < len(parts):
                    metadata["regression_enabled"] = (
                        parts[enabled_idx].lower() == "true"
                    )
            except:
                pass
...
def create_pdf_report(data_file: Path, output_pdf: Path):
    ...
    astrolens_logo = load_and_convert_logo(
        script_dir / "assets" / "astrolens.png", target_height=80
    )
    pyspec_logo = load_and_convert_logo(
        script_dir / "assets" / "icon.png",
        target_height=80,
        convert_yellow_to_white=True,
    )
...
"""
        ...
        if metadata["regression_enabled"]:
            try:
                ...
                interp_fn, interp_kind = make_interpolator(
                    pixels, adcs, method=method, smooth=smooth
                )
        ...
        ax2.plot(
            wavelengths, intensities, "k-", linewidth=0.5, label="Spectroscopy Data"
        )
        ...
        if metadata["regression_enabled"]:
            try:
                ...
                interp_fn, interp_kind = make_interpolator(
                    wavelengths, intensities, method=method, smooth=smooth
                )
        ...
        ax2.set_ylabel(
            "INTENSITY (a.u.)", fontsize=8, weight="bold", family="monospace"
        )
        ax2.set_title(
            "SPECTROSCOPY MODE: WAVELENGTH vs INTENSITY",
            fontsize=9,
            weight="bold",
            family="monospace",
            pad=10,
...
"""
        ...
        ax3.set_title(
            "CALIBRATION CURVE", fontsize=9, weight="bold", family="monospace", pad=10
        )
...
def main():
    ...
    if args.infile:
        ...
        if chosen is None:
            p = input(
                "No file chosen. Enter path to .dat file (or press Enter to quit): "
            ).strip()
            if not p:
